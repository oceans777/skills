...
for skill_name in $SKILL_NAMES; do
  ...
  cat > "$FIRST_PARTY_ROOT/$skill_name/SKILL.md" <<EOF_SKILL
---
name: $skill_name
...
export CLAUDE_HOME
OUTPUT=$(sh "$REPO_ROOT/scripts/install-skills.sh" --runtime claude --first-party-root "$FIRST_PARTY_ROOT" --community-root "$COMMUNITY_ROOT" --without-catalog)
...
OUTPUT=$(sh "$REPO_ROOT/scripts/install-skills.sh" --all-existing-runtimes --first-party-root "$FIRST_PARTY_ROOT" --community-root "$COMMUNITY_ROOT" --without-catalog)
